...
func TestBlockStatesIdFollowsARename(t *testing.T) {
	for _, version := range []types.ProtocolVersion{types.ProtocolVersions.MINECRAFT_1_19_3, types.ProtocolVersions.MINECRAFT_1_19_4, types.ProtocolVersions.MINECRAFT_1_20, types.ProtocolVersions.MINECRAFT_1_20_2} {
		states, err := BlockStatesFor(version)
		if err != nil {
			t.Fatalf("BlockStatesFor() error: %v", err)
		}
