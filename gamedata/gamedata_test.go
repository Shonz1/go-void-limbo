...
// 1.19.3 synchronizes three registries to 1.19.4's six, read off the two
// jars: the dimension types, the biomes and the chat types. 1.19.4 is where
// the damage types and the armor trims appeared, so 1.19.3 declares neither,
// and a compound naming a registry a client does not know is one it refuses.
// The chat types are 1.19.4's entry for entry. The tag sets cover ten
// registries to 1.19.4's eleven, there being no damage types to tag, and
// differ by what 1.19.4 added and by the one biome tag it split in five,
// since each version declares the tags of its own jar. The biome is 1.19.3's
// own, with the climate's precipitation spelled as a name rather than as the
// flag 1.19.4 turned it into.
func TestRegistriesFor1_19_3AreItsOwnAndNot1_19_4s(t *testing.T) {
	registries := func(load func() ([]Registry, error)) map[string]map[string]nbt.Tag {
		data := map[string]map[string]nbt.Tag{}
		for _, registry := range mustLoadRegistries(t, load) {
			data[registry.Name] = map[string]nbt.Tag{}
	...
	older, newer := registries(registriesMinecraft1_19_3), registries(registriesMinecraft1_19_4)

	if len(older) != 3 {
		t.Errorf("1.19.3 is sent %d registries, want 3", len(older))
	}

	for _, name := range []string{"minecraft:dimension_type", "minecraft:worldgen/biome", "minecraft:chat_type"} {
		if _, ok := older[name]; !ok {
			t.Errorf("1.19.3 is not sent %s, which its jar declares", name)
		}
	}

	// The three 1.19.4 introduced, which a 1.19.3 client would refuse.
	for _, name := range []string{"minecraft:damage_type", "minecraft:trim_pattern", "minecraft:trim_material"} {
		if _, ok := older[name]; ok {
			t.Errorf("1.19.3 is sent %s, which 1.19.4 introduced", name)
		}

		if _, ok := newer[name]; !ok {
			t.Errorf("1.19.4 is no longer sent %s, which its jar does declare", name)
		}
	}

	for name, entries := range older {
		if len(entries) == 0 {
			t.Errorf("1.19.3's %s holds no entries, want every registry it declares filled", name)
		}

		for entryName, data := range entries {
			if data == nil {
				t.Errorf("1.19.3's %s entry %s has no definition, which the one-compound shape cannot send", name, entryName)
			}

			if _, ok := newer[name][entryName]; !ok {
				t.Errorf("1.19.3's %s holds %s, which 1.19.4's does not", name, entryName)
			}
		}
	}

	if len(older["minecraft:chat_type"]) != len(newer["minecraft:chat_type"]) {
		t.Errorf("1.19.3's chat types hold %d entries and 1.19.4's %d, want the same: 1.19.4 added none", len(older["minecraft:chat_type"]), len(newer["minecraft:chat_type"]))
	}

	// The climate, spelled each version's own way.
	plains, ok := older["minecraft:worldgen/biome"]["minecraft:plains"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.19.3's plains biome is %T, want a compound", older["minecraft:worldgen/biome"]["minecraft:plains"])
	}

	if precipitation, ok := plains["precipitation"].(nbt.String); !ok || precipitation != "none" {
		t.Errorf("1.19.3's plains biome has precipitation %v, want the name none", plains["precipitation"])
	}

	if _, ok := plains["has_precipitation"]; ok {
		t.Error("1.19.3's plains biome has has_precipitation, a field 1.19.4 introduced")
	}

	if _, ok := newer["minecraft:worldgen/biome"]["minecraft:plains"].(nbt.Compound)["precipitation"]; ok {
		t.Error("1.19.4's plains biome has precipitation, a field 1.19.4 retired")
	}

	// The dimension type the play login names outright, so first is where it
	// has to be here as well.
	dimensionTypes := mustLoadRegistries(t, registriesMinecraft1_19_3)[0]
	if dimensionTypes.Name != "minecraft:dimension_type" || dimensionTypes.Entries[0].Name != "minecraft:overworld" {
		t.Errorf("1.19.3's first registry is %s starting with %s, want minecraft:dimension_type starting with minecraft:overworld", dimensionTypes.Name, dimensionTypes.Entries[0].Name)
	}
	...
	olderTags, newerTags := tags(tagsMinecraft1_19_3), tags(tagsMinecraft1_19_4)

	if len(olderTags)+1 != len(newerTags) {
		t.Errorf("1.19.3 declares tags for %d registries and 1.19.4 for %d, want one fewer for 1.19.3", len(olderTags), len(newerTags))
	}

	if _, ok := olderTags["minecraft:damage_type"]; ok {
		t.Error("1.19.3 declares damage type tags, for a registry 1.19.4 introduced")
	}

	introduced := map[string][]string{
		"minecraft:block":          {"minecraft:smelts_to_glass"},
		"minecraft:item":           {"minecraft:tools", "minecraft:swords", "minecraft:smelts_to_glass"},
		"minecraft:entity_type":    {"minecraft:fall_damage_immune", "minecraft:dismounts_underwater"},
		"minecraft:worldgen/biome": {"minecraft:spawns_gold_rabbits", "minecraft:spawns_white_rabbits", "minecraft:snow_golem_melts"},
	}

	for registry, names := range introduced {
		for _, name := range names {
			if olderTags[registry][name] {
				t.Errorf("1.19.3 declares %s, a %s tag 762 introduced", name, registry)
			}

			if !newerTags[registry][name] {
				t.Errorf("1.19.4 no longer declares %s, a %s tag its jar does declare", name, registry)
			}
		}
	}

	if !olderTags["minecraft:worldgen/biome"]["minecraft:only_allows_snow_and_gold_rabbits"] {
		t.Error("1.19.3 no longer declares only_allows_snow_and_gold_rabbits, a biome tag its jar does declare")
	}

	if newerTags["minecraft:worldgen/biome"]["minecraft:only_allows_snow_and_gold_rabbits"] {
		t.Error("1.19.4 declares only_allows_snow_and_gold_rabbits, a biome tag it split up")
	}
}
...
// 1.18 synchronizes the same two registries as 1.18.2, read off the two
// jars: the dimension types and the biomes, the two its registry access
// marks as sent to the client, 1.18.2 having added none. The dimension type
// is its own for one field: 1.18.2 is where tag keys appeared and the
// infiniburn field took the hashed codec, and 1.18 reads the tag's plain
// name there. The biome is 1.18.2's by reference, the codec being field for
// field the same in the two. The tag sets cover five registries to 1.18.2's
// six, since 1.18.2 is where the biomes came to have tags, and within the
// five declare the same tags name for name but for the one block tag 1.18.2
// added.
func TestRegistriesFor1_18AreItsOwnAndNot1_18_2s(t *testing.T) {
	registries := func(load func() ([]Registry, error)) map[string]map[string]nbt.Tag {
		data := map[string]map[string]nbt.Tag{}
		for _, registry := range mustLoadRegistries(t, load) {
			data[registry.Name] = map[string]nbt.Tag{}
	...
	older, newer := registries(registriesMinecraft1_18), registries(registriesMinecraft1_18_2)

	if len(older) != 2 {
		t.Errorf("1.18 is sent %d registries, want 2", len(older))
	}

	for _, name := range []string{"minecraft:dimension_type", "minecraft:worldgen/biome"} {
		if _, ok := older[name]; !ok {
			t.Errorf("1.18 is not sent %s, which its registry access sends", name)
		}
	}

	for name, entries := range older {
		if len(entries) == 0 {
			t.Errorf("1.18's %s holds no entries, want every registry it declares filled", name)
		}

		for entryName, data := range entries {
			if data == nil {
				t.Errorf("1.18's %s entry %s has no definition, which the one-compound shape cannot send", name, entryName)
			}
		}
	}

	// The dimension type is 1.18.2's with the hash off the infiniburn tag.
	overworld, ok := older["minecraft:dimension_type"]["minecraft:overworld"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.18's overworld is %T, want a compound", older["minecraft:dimension_type"]["minecraft:overworld"])
	}

	newerOverworld, ok := newer["minecraft:dimension_type"]["minecraft:overworld"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.18.2's overworld is %T, want a compound", newer["minecraft:dimension_type"]["minecraft:overworld"])
	}

	if infiniburn, ok := overworld["infiniburn"].(nbt.String); !ok || infiniburn != "minecraft:infiniburn_overworld" {
		t.Errorf("1.18's overworld burns %v, want the plain tag name from before tag keys", overworld["infiniburn"])
	}

	if infiniburn, ok := newerOverworld["infiniburn"].(nbt.String); !ok || infiniburn != "#minecraft:infiniburn_overworld" {
		t.Errorf("1.18.2's overworld burns %v, want the hashed tag key 1.18.2 introduced", newerOverworld["infiniburn"])
	}

	if len(overworld) != len(newerOverworld) {
		t.Errorf("1.18's overworld has %d fields and 1.18.2's %d, want the same: the codecs differ in one field's value, not in their fields", len(overworld), len(newerOverworld))
	}
	...
	for field, value := range overworld {
		...
		if !reflect.DeepEqual(value, newerOverworld[field]) {
			t.Errorf("1.18's overworld has %s = %v and 1.18.2's %v, want the same: the field has the same codec in the two", field, value, newerOverworld[field])
		}
	}

	// The biome is 1.18.2's, category and all.
	plains, ok := older["minecraft:worldgen/biome"]["minecraft:plains"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.18's plains biome is %T, want a compound", older["minecraft:worldgen/biome"]["minecraft:plains"])
	}

	if !reflect.DeepEqual(plains, newer["minecraft:worldgen/biome"]["minecraft:plains"]) {
		t.Error("1.18's plains biome differs from 1.18.2's, want the same: the codec is field for field the same in the two")
	}

	if category, ok := plains["category"].(nbt.String); !ok || category != "plains" {
		t.Errorf("1.18's plains biome has category %v, want plains, a field 1.18 reads as 1.18.2 does", plains["category"])
	}

	// The dimension type the play login spells out, so first is where it
	// has to be here as well.
	dimensionTypes := mustLoadRegistries(t, registriesMinecraft1_18)[0]
	if dimensionTypes.Name != "minecraft:dimension_type" || dimensionTypes.Entries[0].Name != "minecraft:overworld" {
		t.Errorf("1.18's first registry is %s starting with %s, want minecraft:dimension_type starting with minecraft:overworld", dimensionTypes.Name, dimensionTypes.Entries[0].Name)
	}

	olderTags, newerTags := mustLoadTags(t, tagsMinecraft1_18), mustLoadTags(t, tagsMinecraft1_18_2)
	if len(olderTags) != 5 {
		t.Errorf("1.18 declares tags for %d registries, want five", len(olderTags))
	}

	if len(newerTags) != 6 {
		t.Errorf("1.18.2 declares tags for %d registries, want six", len(newerTags))
	}
	...
	for _, registry := range []string{"minecraft:block", "minecraft:item", "minecraft:entity_type", "minecraft:fluid", "minecraft:game_event"} {
		if _, ok := olderNames[registry]; !ok {
			t.Errorf("1.18 declares no %s tags, which its jar does declare and a 1.18 server sends", registry)
		}

		if _, ok := newerNames[registry]; !ok {
			t.Errorf("1.18.2 declares no %s tags, which its jar does declare and a 1.18.2 server sends", registry)
		}

		for name := range olderNames[registry] {
			if !newerNames[registry][name] {
				t.Errorf("1.18 declares %s, a %s tag 1.18.2 does not", name, registry)
			}
		}

		for name := range newerNames[registry] {
			if !olderNames[registry][name] && name != "minecraft:fall_damage_resetting" {
				t.Errorf("1.18.2 declares %s, a %s tag 1.18 does not, and not the one 1.18.2 added", name, registry)
			}
		}
	}

	if _, ok := olderNames["minecraft:worldgen/biome"]; ok {
		t.Error("1.18 declares biome tags, which 1.18.2 introduced")
	}

	if olderNames["minecraft:block"]["minecraft:fall_damage_resetting"] {
		t.Error("1.18 declares fall_damage_resetting, a block tag 1.18.2 introduced")
	}

	if !newerNames["minecraft:block"]["minecraft:fall_damage_resetting"] {
		t.Error("1.18.2 no longer declares fall_damage_resetting, a block tag its jar does declare")
	}

	// The tags a 1.18 client's own helpers name, which it checks the payload
	// for and leaves over a missing one: a few from each of the five, the
	// ones the client's code reads first.
	for registry, names := range map[string][]string{
		"minecraft:block":       {"minecraft:wool", "minecraft:climbable", "minecraft:infiniburn_overworld", "minecraft:fall_damage_resetting"},
		"minecraft:item":        {"minecraft:wool", "minecraft:arrows", "minecraft:freeze_immune_wearables"},
		"minecraft:entity_type": {"minecraft:skeletons", "minecraft:freeze_immune_entity_types", "minecraft:powder_snow_walkable_mobs"},
		"minecraft:fluid":       {"minecraft:water", "minecraft:lava"},
		"minecraft:game_event":  {"minecraft:vibrations", "minecraft:ignore_vibrations_sneaking"},
	} {
		for _, name := range names {
			if name == "minecraft:fall_damage_resetting" {
				continue
			}

			if !olderNames[registry][name] {
				t.Errorf("1.18 declares no %s %s tag, which its client checks for and leaves without", registry, name)
			}
		}
	}
}

// 1.17.1 synchronizes the same two registries as 1.18, read off the two
// jars: the dimension types and the biomes, the two its registry access
// marks as sent to the client. The dimension type is 1.18's by reference,
// the codec being field for field the same. The biome is its own: 1.18 is
// where the depth and the scale left the codec, and 1.17.1 reads both as
// required fields. The tag sets cover the same five registries, with the
// names 1.17.1's own jar declares: 1.18's additions are absent, and the one
// tag 1.18 renamed keeps its older name.
func TestRegistriesFor1_17_1AreItsOwnAndNot1_18s(t *testing.T) {
	registries := func(load func() ([]Registry, error)) map[string]map[string]nbt.Tag {
		data := map[string]map[string]nbt.Tag{}
		for _, registry := range mustLoadRegistries(t, load) {
			data[registry.Name] = map[string]nbt.Tag{}
	...
	older, newer := registries(registriesMinecraft1_17_1), registries(registriesMinecraft1_18)

	if len(older) != 2 {
		t.Errorf("1.17.1 is sent %d registries, want 2", len(older))
	}

	for _, name := range []string{"minecraft:dimension_type", "minecraft:worldgen/biome"} {
		if _, ok := older[name]; !ok {
			t.Errorf("1.17.1 is not sent %s, which its registry access sends", name)
		}
	}

	for name, entries := range older {
		if len(entries) == 0 {
			t.Errorf("1.17.1's %s holds no entries, want every registry it declares filled", name)
		}

		for entryName, data := range entries {
			if data == nil {
				t.Errorf("1.17.1's %s entry %s has no definition, which the one-compound shape cannot send", name, entryName)
			}
		}
	}

	// The dimension type is 1.18's, plain infiniburn tag and all.
	overworld, ok := older["minecraft:dimension_type"]["minecraft:overworld"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.17.1's overworld is %T, want a compound", older["minecraft:dimension_type"]["minecraft:overworld"])
	}

	if !reflect.DeepEqual(overworld, newer["minecraft:dimension_type"]["minecraft:overworld"]) {
		t.Error("1.17.1's overworld differs from 1.18's, want the same: the codec is field for field the same in the two")
	}

	if infiniburn, ok := overworld["infiniburn"].(nbt.String); !ok || infiniburn != "minecraft:infiniburn_overworld" {
		t.Errorf("1.17.1's overworld burns %v, want the plain tag name from before tag keys", overworld["infiniburn"])
	}

	// The biome is 1.18's with the depth and the scale on top.
	plains, ok := older["minecraft:worldgen/biome"]["minecraft:plains"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.17.1's plains biome is %T, want a compound", older["minecraft:worldgen/biome"]["minecraft:plains"])
	}

	newerPlains, ok := newer["minecraft:worldgen/biome"]["minecraft:plains"].(nbt.Compound)
	if !ok {
		t.Fatalf("1.18's plains biome is %T, want a compound", newer["minecraft:worldgen/biome"]["minecraft:plains"])
	}

	if depth, ok := plains["depth"].(nbt.Float); !ok || depth != 0.125 {
		t.Errorf("1.17.1's plains biome has depth %v, want the 0.125 its client's own plains carries", plains["depth"])
	}

	if scale, ok := plains["scale"].(nbt.Float); !ok || scale != 0.05 {
		t.Errorf("1.17.1's plains biome has scale %v, want the 0.05 its client's own plains carries", plains["scale"])
	}

	for _, field := range []string{"depth", "scale"} {
		if _, ok := newerPlains[field]; ok {
			t.Errorf("1.18's plains biome carries %s, a field 1.18 took off the codec", field)
		}
	}

	if len(plains) != len(newerPlains)+2 {
		t.Errorf("1.17.1's plains biome has %d fields and 1.18's %d, want two more for 1.17.1", len(plains), len(newerPlains))
	}

	for field, value := range newerPlains {
		if !reflect.DeepEqual(value, plains[field]) {
			t.Errorf("1.17.1's plains biome has %s = %v and 1.18's %v, want the same: the field has the same codec in the two", field, plains[field], value)
		}
	}

	// The dimension type the play login spells out, so first is where it
	// has to be here as well.
	dimensionTypes := mustLoadRegistries(t, registriesMinecraft1_17_1)[0]
	if dimensionTypes.Name != "minecraft:dimension_type" || dimensionTypes.Entries[0].Name != "minecraft:overworld" {
		t.Errorf("1.17.1's first registry is %s starting with %s, want minecraft:dimension_type starting with minecraft:overworld", dimensionTypes.Name, dimensionTypes.Entries[0].Name)
	}

	olderTags, newerTags := mustLoadTags(t, tagsMinecraft1_17_1), mustLoadTags(t, tagsMinecraft1_18)
	if len(olderTags) != 5 {
		t.Errorf("1.17.1 declares tags for %d registries, want five", len(olderTags))
	}
	...
	// What 1.18 added to the two sets that grew, and the one tag it renamed.
	added := map[string]map[string]bool{
		"minecraft:block": {
			"minecraft:animals_spawnable_on": true, "minecraft:axolotls_spawnable_on": true, "minecraft:azalea_grows_on": true,
			"minecraft:azalea_root_replaceable": true, "minecraft:big_dripleaf_placeable": true, "minecraft:foxes_spawnable_on": true,
			"minecraft:goats_spawnable_on": true, "minecraft:mooshrooms_spawnable_on": true, "minecraft:parrots_spawnable_on": true,
			"minecraft:polar_bears_spawnable_on_in_frozen_ocean": true, "minecraft:rabbits_spawnable_on": true,
			"minecraft:replaceable_plants": true, "minecraft:terracotta": true, "minecraft:wolves_spawnable_on": true,
			"minecraft:lava_pool_stone_cannot_replace": true,
		},
		"minecraft:item": {"minecraft:dirt": true, "minecraft:terracotta": true},
	}

	for _, registry := range []string{"minecraft:block", "minecraft:item", "minecraft:entity_type", "minecraft:fluid", "minecraft:game_event"} {
		if _, ok := olderNames[registry]; !ok {
			t.Errorf("1.17.1 declares no %s tags, which its jar does declare and a 1.17.1 server sends", registry)
		}

		for name := range olderNames[registry] {
			if !newerNames[registry][name] && name != "minecraft:lava_pool_stone_replaceables" {
				t.Errorf("1.17.1 declares %s, a %s tag 1.18 does not, and not the one 1.18 renamed", name, registry)
			}
		}

		for name := range newerNames[registry] {
			if !olderNames[registry][name] && !added[registry][name] {
				t.Errorf("1.18 declares %s, a %s tag 1.17.1 does not, and not one 1.18 added", name, registry)
			}
		}

		for name := range added[registry] {
			if olderNames[registry][name] {
				t.Errorf("1.17.1 declares %s, a %s tag 1.18 added", name, registry)
			}
		}
	}

	if !olderNames["minecraft:block"]["minecraft:lava_pool_stone_replaceables"] {
		t.Error("1.17.1 no longer declares lava_pool_stone_replaceables, the block tag its jar declares under the older name")
	}

	// The tags a 1.17.1 client's own helpers name, which it checks the
	// payload for and leaves over a missing one: a few from each of the five,
	// the ones the client's code reads first.
	for registry, names := range map[string][]string{
		"minecraft:block":       {"minecraft:wool", "minecraft:climbable", "minecraft:infiniburn_overworld", "minecraft:lava_pool_stone_replaceables"},
		"minecraft:item":        {"minecraft:wool", "minecraft:arrows", "minecraft:freeze_immune_wearables"},
		"minecraft:entity_type": {"minecraft:skeletons", "minecraft:freeze_immune_entity_types", "minecraft:powder_snow_walkable_mobs"},
		"minecraft:fluid":       {"minecraft:water", "minecraft:lava"},
		"minecraft:game_event":  {"minecraft:vibrations", "minecraft:ignore_vibrations_sneaking"},
		...
		for _, name := range names {
			if !olderNames[registry][name] {
				t.Errorf("1.17.1 declares no %s %s tag, which its client checks for and leaves without", registry, name)
			}
		}
	}
}
...
// entry for entry, 1.19.3 having moved them from code into data files
// without changing one. The tag sets cover the same ten registries, and
// differ by what each jar declares, since each version declares the tags of
// its own. The dimension type and the biome are 1.19.3's by reference, since
// the codecs are field-for-field identical in the two.
func TestRegistriesFor1_19_1AreItsOwnAndNot1_19_3s(t *testing.T) {
	registries := func(load func() ([]Registry, error)) map[string]map[string]nbt.Tag {
		data := map[string]map[string]nbt.Tag{}
		for _, registry := range mustLoadRegistries(t, load) {
			data[registry.Name] = map[string]nbt.Tag{}
	...
	older, newer := registries(registriesMinecraft1_19_1), registries(registriesMinecraft1_19_3)

	if len(older) != 3 {
		t.Errorf("1.19.1 is sent %d registries, want 3", len(older))
	}

	for _, name := range []string{"minecraft:dimension_type", "minecraft:worldgen/biome", "minecraft:chat_type"} {
		if _, ok := older[name]; !ok {
			t.Errorf("1.19.1 is not sent %s, which its registry access sends", name)
		}
	}

	for name, entries := range older {
		if len(entries) == 0 {
			t.Errorf("1.19.1's %s holds no entries, want every registry it declares filled", name)
		}

		for entryName, data := range entries {
			if data == nil {
				t.Errorf("1.19.1's %s entry %s has no definition, which the one-compound shape cannot send", name, entryName)
			}

			if !reflect.DeepEqual(newer[name][entryName], data) {
				t.Errorf("1.19.1's %s entry %s differs from 1.19.3's, want the same: 1.19.3 changed none of them", name, entryName)
			}
		}
	}

	if len(older["minecraft:chat_type"]) != len(newer["minecraft:chat_type"]) {
		t.Errorf("1.19.1's chat types hold %d entries and 1.19.3's %d, want the same: 1.19.3 added none", len(older["minecraft:chat_type"]), len(newer["minecraft:chat_type"]))
	}

	// The dimension type the play login names outright, so first is where it
	// has to be here as well.
	dimensionTypes := mustLoadRegistries(t, registriesMinecraft1_19_1)[0]
	if dimensionTypes.Name != "minecraft:dimension_type" || dimensionTypes.Entries[0].Name != "minecraft:overworld" {
		t.Errorf("1.19.1's first registry is %s starting with %s, want minecraft:dimension_type starting with minecraft:overworld", dimensionTypes.Name, dimensionTypes.Entries[0].Name)
	}
	...
	olderTags, newerTags := tags(tagsMinecraft1_19_1), tags(tagsMinecraft1_19_3)

	if len(olderTags) != len(newerTags) {
		t.Errorf("1.19.1 declares tags for %d registries and 1.19.3 for %d, want the same ten", len(olderTags), len(newerTags))
	}

	for registry := range newerTags {
		if _, ok := olderTags[registry]; !ok {
			t.Errorf("1.19.1 declares no %s tags, which 1.19.3 does and its jar does too", registry)
		}
	}

	introduced := map[string][]string{
		"minecraft:block": {"minecraft:all_signs", "minecraft:invalid_spawn_inside"},
		"minecraft:item":  {"minecraft:creeper_igniters", "minecraft:fence_gates"},
	}

	for registry, names := range introduced {
		for _, name := range names {
			if olderTags[registry][name] {
				t.Errorf("1.19.1 declares %s, a %s tag 761 introduced", name, registry)
			}

			if !newerTags[registry][name] {
				t.Errorf("1.19.3 no longer declares %s, a %s tag its jar does declare", name, registry)
			}
		}
	}

	retired := map[string]string{
		"minecraft:block": "minecraft:non_flammable_wood",
		"minecraft:item":  "minecraft:overworld_natural_logs",
	}

	for registry, name := range retired {
		if !olderTags[registry][name] {
			t.Errorf("1.19.1 no longer declares %s, a %s tag its jar does declare", name, registry)
		}

		if newerTags[registry][name] {
			t.Errorf("1.19.3 declares %s, a %s tag it retired", name, registry)
		}
	}
}
...
func TestProviderGivesEachVersionItsOwnRegistries(t *testing.T) {
	...
	floor := provider.PacketsFor(types.ProtocolVersions.MINECRAFT_1_17_1)
	bottom := provider.PacketsFor(types.ProtocolVersions.MINECRAFT_1_18)
	...
	// 1.19.3 likewise, with a compound of its own: three registries to
	// 1.19.4's six, which the size sees.
	if len(oldest000000) != 1 {
		t.Errorf("1.19.3 was sent %d packets, want the tags alone", len(oldest000000))
	}

	oldestCodec := provider.RegistryCodecFor(types.ProtocolVersions.MINECRAFT_1_19_3)
	if len(oldestCodec) == 0 {
		t.Error("1.19.3 has no registry codec, want the compound its play login carries")
	}

	if len(oldestCodec) >= len(olderCodec) {
		t.Errorf("1.19.3's registry codec is %d bytes and 1.19.4's %d, want fewer for 1.19.3", len(oldestCodec), len(olderCodec))
	}

	// 1.19.1 likewise, with a compound of its own that holds the same three
	// registries with the same entries, which the size sees as well.
	if len(oldest0000000) != 1 {
		t.Errorf("1.19.1 was sent %d packets, want the tags alone", len(oldest0000000))
	}

	firstCodec := provider.RegistryCodecFor(types.ProtocolVersions.MINECRAFT_1_19_1)
	if len(firstCodec) == 0 {
		t.Error("1.19.1 has no registry codec, want the compound its play login carries")
	}
	...
	// 1.18 likewise, with a compound the size of 1.18.2's -- the same two
	// registries, one field differing by a character -- and a dimension
	// type spelled out of its own, which the content test tells from
	// 1.18.2's.
	if len(bottom) != 1 {
		t.Errorf("1.18 was sent %d packets, want the tags alone", len(bottom))
	}

	bottomCodec := provider.RegistryCodecFor(types.ProtocolVersions.MINECRAFT_1_18)
	if len(bottomCodec) == 0 {
		t.Error("1.18 has no registry codec, want the compound its play login carries")
	}

	if len(bottomCodec) != len(lowestCodec)-1 {
		t.Errorf("1.18's registry codec is %d bytes and 1.18.2's %d, want one fewer for 1.18: the hash off the infiniburn tag", len(bottomCodec), len(lowestCodec))
	}

	if bytes.Equal(bottomCodec, lowestCodec) {
		t.Error("1.18 has 1.18.2's registry codec, want its own")
	}

	if dimensionType := provider.DimensionTypeFor(types.ProtocolVersions.MINECRAFT_1_18); len(dimensionType) == 0 {
		t.Error("1.18 has no dimension type, want the entry its play login spells out")
	} else if bytes.Equal(dimensionType, provider.DimensionTypeFor(types.ProtocolVersions.MINECRAFT_1_18_2)) {
		t.Error("1.18 has 1.18.2's dimension type, want its own")
	}

	// 1.17.1 likewise, with a compound larger than 1.18's -- the same two
	// registries, the biome carrying the depth and the scale 1.18 took off
	// its codec -- and 1.18's dimension type spelled out, the codec being the
	// same in the two.
	if len(floor) != 1 {
		t.Errorf("1.17.1 was sent %d packets, want the tags alone", len(floor))
	}

	floorCodec := provider.RegistryCodecFor(types.ProtocolVersions.MINECRAFT_1_17_1)
	if len(floorCodec) == 0 {
		t.Error("1.17.1 has no registry codec, want the compound its play login carries")
	}

	if len(floorCodec) <= len(bottomCodec) {
		t.Errorf("1.17.1's registry codec is %d bytes and 1.18's %d, want more for 1.17.1: the depth and the scale on the biome", len(floorCodec), len(bottomCodec))
	}

	if dimensionType := provider.DimensionTypeFor(types.ProtocolVersions.MINECRAFT_1_17_1); len(dimensionType) == 0 {
		t.Error("1.17.1 has no dimension type, want the entry its play login spells out")
	} else if !bytes.Equal(dimensionType, provider.DimensionTypeFor(types.ProtocolVersions.MINECRAFT_1_18)) {
	...
	for _, version := range types.SupportedProtocolVersions[3:] {
