...
// passes through the phase, and the twenty-one versions below it go from their
// login straight into play.
func TestHasConfigurationPhase(t *testing.T) {
	for _, version := range SupportedProtocolVersions[:21] {
...
func TestMaySignEncryptionChallenge(t *testing.T) {
	...
	for _, version := range SupportedProtocolVersions[18:] {
