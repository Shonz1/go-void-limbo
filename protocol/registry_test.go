...
// dimensionType1_18_2 stands in for the dimension type a 1.18.2 play login
// spells out, which comes from the same source as the registries and only
// for that version.
var dimensionType1_18_2 = []byte{0x0A, 0x00, 0x00, 0x03, 0x00, 0x05, 'm', 'i', 'n', '_', 'y', 0xFF, 0xFF, 0xFF, 0xC0, 0x00}

func (r registryCodecs) DimensionTypeFor(version types.ProtocolVersion) []byte {
	if version.ID < types.ProtocolVersions.MINECRAFT_1_19.ID && r[version.ID] != nil {
		return dimensionType1_18_2
	}
...
// 1.19.1's, a 1.19 login 1.19's, and a 1.18.2 login 1.18.2's, with the
// dimension type it spells out from the same source -- and
...
func TestEncodeClientboundWritesTheRegistriesIntoALoginBefore1_20_2(t *testing.T) {
	...
	for _, version := range types.SupportedProtocolVersions[:6] {
		...
		// 1.18.2 spells the dimension type out behind the registries, and no
		// other version does.
		if spelled, want := bytes.Contains(body, dimensionType1_18_2), version.ID == types.ProtocolVersions.MINECRAFT_1_18_2.ID; spelled != want {
			t.Errorf("protocol %d: the login spells the dimension type out: %t, want %t", version.ID, spelled, want)
		}
	...
	// 1.19's the 1.19 login, and one with 1.19's and not 1.18.2's the 1.18.2
